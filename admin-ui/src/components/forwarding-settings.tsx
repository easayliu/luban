...
function LearnedRejections() {
  ...
  return (
    <SettingsGroup
      ...
      description={t(
        '上游点名过的组合会被记下来：某模型不收某取值的（400），下次本地直接拒；某模型已废弃某参数的（400），下次转发前剥掉；某模型对「无 tools 的单条消息 + 某个 max_tokens」回过 200 却零输出的，同类下次本地直接拒；某模型拒答过（stop_reason refusal，如 cyber 类内容）的那条提示词，逐字相同的重发本地直接拒——只拦那一条内容，同形态的其他请求不受影响（两条都是 403，随「拒绝探针请求」开关），规则文案里是上游当时回复的开头。规则落库、重启保留，7 天后自动丢弃重新验证。上游放开了而本地还在拦时，在这里删掉即可。',
        'Combinations upstream has called out are remembered: a value a model refuses (400) is rejected locally next time; a parameter a model deprecated (400) is stripped before forwarding; a tool-less single-message request class (model + max_tokens) that upstream answered with 200 and zero output tokens is rejected locally next time; a prompt upstream refused (stop_reason refusal, e.g. cyber content) is rejected locally when resent verbatim, and only that one prompt, never other requests of the same shape (both 403, governed by "Reject probe requests"), with the start of that upstream reply kept as the rule text. Rules persist across restarts and expire after 7 days. If upstream has since allowed something, remove the rule here.',
      )}
    >
      {query.isPending ? (
